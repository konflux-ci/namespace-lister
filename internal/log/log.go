...
func GetLogLevel() slog.Level {
	env := os.Getenv(constant.EnvLogLevel)
...
func SetLoggerIntoContext(ctx context.Context, logger *slog.Logger) context.Context {
	return context.WithValue(ctx, nscontext.ContextKeyLogger, logger)
}
...
func GetLoggerFromContext(ctx context.Context) *slog.Logger {
	if l, ok := ctx.Value(nscontext.ContextKeyLogger).(*slog.Logger); ok && l != nil {
		return l
	}
